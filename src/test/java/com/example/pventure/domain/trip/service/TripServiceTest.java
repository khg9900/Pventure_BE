...
class TripServiceTest {
    ...
    @Mock
    private TripFolderService tripFolderService;
    ...
    void getTrips_Success() {
        ...
        when(memberService.getMemberSummaryDtoList(trip))
                .thenReturn(List.of(memberDto));

        // when
        List<TripResponseDto> result = tripService.getTrips(1L, searchRequest);
    ...
    void getTrip_Success() {
        ...
        // when
        TripResponseDto response = tripService.getTrip(1L, 1L);
    ...
    void getTrip_Unauthorized_Throws() {
        ...
        // when & then
        ApiException ex = assertThrows(ApiException.class, () -> tripService.getTrip(1L, 1L));

        assertThat(ex.getErrorCode()).isEqualTo(ErrorCode.UNAUTHORIZED_MEMBER_ACCESS);
        assertThat(ex.getMessage()).contains(ErrorCode.UNAUTHORIZED_MEMBER_ACCESS.getMessage());
        verify(memberService, times(1)).isMember(user, trip);
    }
    ...
    void updateTrip_Success() {
        ...
        // when
        TripResponseDto response = tripService.updateTrip(1L, 1L, updateDto);
    ...
    void updateTrip_Unauthorized_Throws() {
        ...
        ApiException ex = assertThrows(ApiException.class, () ->
                tripService.updateTrip(1L, 1L, requestDto)
        );
